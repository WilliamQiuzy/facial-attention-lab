#!/usr/bin/env python3
"""Run the frozen twenty-model broad literature shared V9 screen on H200."""
...
from src.evaluation.broad_literature_shared_search_v9 import (  # noqa: E402
...
def _validate_source_metrics(metrics: object) -> None:
    if type(metrics) is not dict or set(metrics) != set(SOURCES):
        raise ValueError("broad V9 metrics require the exact three sources")
    for source in SOURCES:
        ...
        if (
            ...
            raise ValueError("broad V9 metric rows must be closed and finite")
...
def build_report(
    *,
    ...
    expected = tuple(row.candidate_id for row in candidate_registry_v9())
    if type(ensemble_metrics) is not dict or tuple(ensemble_metrics) != expected:
        raise ValueError("ensemble metrics must match all broad V9 candidates")
    ...
    if (
        ...
        raise ValueError("broad V9 counts, runtime, or commitments drifted")

    summaries = {}
    comparator = ensemble_metrics["BLV9-000"]
    for candidate_id in expected:
        ...
        summaries[candidate_id] = {
            ...
            "promotion_gate_passed": (
                candidate_id != "BLV9-000"
                and candidate_is_promotable(observed, dict(comparator))
            ),
        }
    ...
    registry = candidate_registry_v9()
    return {
        "schema_version": "broad_literature_shared_v9_search",
        ...
        "candidate_registry": [
            {
                ...
                "settings": {name: value for name, value in row.settings},
                "inference_change": row.inference_change,
            }
            for row in registry
        ],
        ...
        "summaries": summaries,
        "selection": {
            "evaluated_new_models": 20,
            "comparator": "BLV9-000_exact_RSR8-001",
            "seeds": list(SEEDS),
...
def write_release_atomic(output: Path, report: Mapping[str, object]) -> str:
    if not isinstance(output, Path) or type(report) is not dict:
        raise ValueError("release publication requires a Path and exact report mapping")
...
def _serialize(result) -> dict[str, object]:
    return {
        ...
        "active_mechanism": result.active_mechanism,
    }
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("the frozen broad V9 search requires the verified H200")
    ...
    if counts != _COUNTS:
        raise ValueError("broad V9 participant counts drifted")
    ...
    for candidate in candidate_registry_v9():
        ...
        for seed in SEEDS:
            print(
                f"START {candidate.candidate_id} {candidate.mechanism} seed={seed}",
                flush=True,
            )
            result = evaluate_broad_literature_candidate(
