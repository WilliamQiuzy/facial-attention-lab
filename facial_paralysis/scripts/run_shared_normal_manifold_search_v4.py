#!/usr/bin/env python3
"""Run the frozen shared normal-manifold search on the verified H200."""
...
from src.evaluation.shared_normal_manifold_search_v4 import (  # noqa: E402
    evaluate_normal_manifold_candidate,
    rank_normal_manifold_results,
)
from src.models.normal_manifold_candidate_registry_v4 import (  # noqa: E402
...
_METRIC_KEYS = {
    "accuracy", "balanced_accuracy", "auroc", "sensitivity", "specificity", "brier"
}


def validate_candidate_phase(phase: str, candidate_ids: tuple[str, ...]) -> None:
    all_ids = tuple(candidate.candidate_id for candidate in candidate_registry_v4())
    if (
        ...
        raise ValueError("candidate identifiers differ from the frozen v4 registry")
    if phase == "screen":
    ...
    if not valid:
        raise ValueError("candidate set differs from the frozen v4 phase")
...
def build_report(
    *,
    phase: str,
    evaluations: Mapping[str, Mapping[str, Mapping[str, float]]],
    gradient_cosines: Mapping[str, Mapping[str, float]],
    ranking: tuple[str, ...],
    counts: Mapping[str, int],
    runtime: Mapping[str, object],
    commitments: Mapping[str, str],
) -> dict[str, object]:
    if type(evaluations) is not dict or type(gradient_cosines) is not dict:
        raise ValueError("evaluations and gradient audits must be exact dictionaries")
    candidate_ids = tuple(evaluations)
    validate_candidate_phase(phase, candidate_ids)
    if (
        set(gradient_cosines) != set(candidate_ids)
        ...
        raise ValueError("v4 report ranking, gradients, or counts drifted")
    for candidate_id in candidate_ids:
        metrics = evaluations[candidate_id]
        cosines = gradient_cosines[candidate_id]
        if (
            set(metrics) != set(SOURCES)
            or any(
                set(metrics[source]) != _METRIC_KEYS
                or any(not np.isfinite(float(value)) for value in metrics[source].values())
                for source in SOURCES
            )
            or set(cosines) != _COSINE_KEYS
            or any(not np.isfinite(float(value)) or float(value) < -1.0 or float(value) > 1.0
                   for value in cosines.values())
        ):
            raise ValueError("v4 metrics or gradient cosines differ from the closed schema")
    return {
        "schema_version": "shared_normal_manifold_router_v4_search",
        "status": "exposed_development_candidate_search_not_clinically_validated",
        "phase": phase,
        "model": {
            "name": "Shared Normal-Manifold Phenotype Router v4",
            "locked_backbone": "MSC2-022",
            "inputs": ["clinical_110d", "dense_mediapipe_478x3"],
            "shared_patient_embedding_dim": 64,
            "shared_normal_anchor": True,
            "source_specific_layers": ["final_binary_head"],
            "source_identifier_input": False,
        },
        "medical_gate": {
            "rationales": NORMAL_MANIFOLD_RATIONALE,
            "controls_only_in_normal_compactness": True,
        ...
        "candidate_registry": [
            {
                ...
                "normal_weight": candidate.normal_weight,
                "universal_blend": candidate.universal_blend,
            }
            for candidate in candidate_registry_v4()
        ],
        ...
        "gradient_cosines": {
            candidate_id: {
                pair: float(value)
                for pair, value in gradient_cosines[candidate_id].items()
            }
            for candidate_id in candidate_ids
        },
        "selection": {
            ...
            "pass_gate": {
                "three_seed_minimum_balanced_accuracy": 0.90,
                "three_seed_minimum_specificity": 0.85,
            },
        },
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.get_device_name(0) != "NVIDIA H200":
        raise RuntimeError("v4 search requires the verified NVIDIA H200")
    registry = candidate_registry_v4()
    ...
    if args.epochs != 20 or args.folds != 6:
        raise ValueError("the frozen v4 search requires 20 updates and six folds")
    ...
    if counts != {"palsynet": 38, "neuroface": 36, "meei": 56}:
        raise ValueError("shared v4 participant counts drifted")
    raw_results = {}
    evaluations = {}
    gradient_cosines = {}
    for candidate_id in candidate_ids:
        ...
        gradient_cosines[candidate_id] = result.gradient_cosines
    ranking = (
        rank_normal_manifold_results(raw_results)
