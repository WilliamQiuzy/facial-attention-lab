#!/usr/bin/env python3
"""Run the deterministic specificity-first shared mechanism Router V9 search."""
...
from src.evaluation.shared_mechanism_search_v9 import (  # noqa: E402
    evaluate_mechanism_candidate,
    rank_mechanism_results,
)
from src.models.shared_mechanism_router_v9 import candidate_registry_v9  # noqa: E402
...
def _ranking_key(candidate_id: str, evaluations: Mapping[str, Mapping[str, object]]):
    ...
    feasible = all(float(metrics[source]["sensitivity"]) >= 0.85 for source in SOURCES)
    return (
        not feasible,
...
def build_report(
    *, evaluations: Mapping[str, Mapping[str, object]], ranking: tuple[str, ...],
    ...
    if (
        ...
        raise ValueError("shared mechanism report registry or commitments drifted")
    for candidate_id in ids:
        ...
        if (
            ...
            raise ValueError("shared mechanism evaluation schema drifted")
        for source in SOURCES:
            metrics = row["metrics"][source]
            if set(metrics) != _METRICS or any(
                not np.isfinite(float(value)) for value in metrics.values()
            ):
                raise ValueError("shared mechanism metrics are incomplete")
    expected = tuple(sorted(ids, key=lambda value: _ranking_key(value, evaluations)))
    if ranking != expected:
        raise ValueError("shared mechanism ranking differs from its frozen objective")
    return {
        "schema_version": "shared_mechanism_router_v9_search",
        ...
        "model": {
            "shared_middle_layer": "one_action_encoder_and_patient_projection",
            "input": "110D_action_geometry_plus_optional_478D_excursion_velocity",
            "source_identifier_input_to_action_encoder": False,
...
def _implementation_sha256() -> str:
    paths = (
        ...
        PROJECT_ROOT / "src/evaluation/shared_mechanism_search_v9.py",
        PROJECT_ROOT / "src/models/shared_mechanism_router_v9.py",
...
def main() -> None:
    ...
    if (
        ...
        or args.epochs != 30 or args.folds != 6
    ):
        raise RuntimeError("shared mechanism V9 requires the deterministic H200 schedule")
    ...
    if counts != _COUNTS:
        raise ValueError("shared mechanism participant counts drifted")
    results = {
        candidate.candidate_id: evaluate_mechanism_candidate(
    ...
    report = build_report(
        evaluations=evaluations, ranking=rank_mechanism_results(results), counts=counts,
        runtime={
