#!/usr/bin/env python3
"""Run the deterministic specificity-first shared Router V9 distillation search."""
...
from src.models.distilled_shared_candidate_registry_v9 import (  # noqa: E402
...
def _ranking_key(
    ...
    comparator = evaluations["DSR9-000"]["metrics"]
    feasible = all(
...
def build_report(
    *,
    ...
    if (
        ...
        raise ValueError("distilled V9 report registry or commitments drifted")
    for candidate_id in ids:
        row = evaluations[candidate_id]
        if (
            set(row) != {
                "metrics", "model_fits", "teacher_self_training_rows",
                "outer_held_teacher_reads", "task_specific_parameter_fraction",
            }
            or int(row["model_fits"]) != 6
            or int(row["teacher_self_training_rows"]) != 0
            or int(row["outer_held_teacher_reads"]) != 0
            or not 0 <= float(row["task_specific_parameter_fraction"]) < 0.10
            or set(row["metrics"]) != set(SOURCES)
        ):
            raise ValueError("distilled V9 evaluation schema drifted")
        for source in SOURCES:
            metrics = row["metrics"][source]
            if set(metrics) != _METRICS or any(
                not np.isfinite(float(value)) for value in metrics.values()
            ):
                raise ValueError("distilled V9 metrics are incomplete")
    expected = tuple(sorted(ids, key=lambda value: _ranking_key(value, evaluations)))
    if ranking != expected:
        raise ValueError("distilled V9 ranking differs from the frozen objective")
    return {
        "schema_version": "distilled_shared_router_v9_search",
        "status": "participant_disjoint_development_search_not_clinically_validated",
        "candidate_registry": [
            {
                "candidate_id": row.candidate_id,
                "teacher_mode": row.teacher_mode,
                "distillation_weight": row.distillation_weight,
                "medical_rationale": row.medical_rationale,
            }
            for row in registry
        ],
        "model": {
            "shared_encoder": "RSR8-001_478D_plus_110D_motor_encoder",
            "teacher_at_inference": False,
            "source_identifier_input_to_shared_encoder": False,
            "task_specific_parameter_fraction_maximum": 0.10,
        },
        "training": {
            "teacher_targets": "outer_train_only_inner_participant_disjoint_oof",
            "teacher_feature_selection": "inner_training_only",
            "outer_held_teacher_reads": 0,
            "deterministic_cuda_required": True,
        },
        "counts": dict(counts),
        ...
        "audit": {
            ...
            "mayo_reads": 0,
            "mayo_predictions": 0,
            "patient_level_rows_published": 0,
        },
        "decision": {
...
def _implementation_sha256() -> str:
    paths = (
        ...
        PROJECT_ROOT / "src/models/distilled_shared_candidate_registry_v9.py",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--teacher-inner-folds", type=int, default=4)
...
def main() -> None:
    ...
    if (
        ...
        or args.epochs != 20
        or args.folds != 6
        or args.teacher_inner_folds != 4
    ):
        raise RuntimeError("distilled V9 requires the frozen deterministic H200 schedule")
    ...
    if counts != _COUNTS:
        raise ValueError("distilled V9 participant counts drifted")
    results = {
        candidate.candidate_id: evaluate_distilled_candidate(
            dataset, candidate, epochs=args.epochs, n_splits=args.folds,
            teacher_inner_splits=args.teacher_inner_folds, seed=0, device="cuda",
        )
    ...
    evaluations = {
        candidate_id: {
            ...
            "teacher_self_training_rows": result.teacher_self_training_rows,
            "outer_held_teacher_reads": result.outer_held_teacher_reads,
            "task_specific_parameter_fraction": result.task_specific_parameter_fraction,
        }
        for candidate_id, result in results.items()
    }
    ranking = rank_distilled_results(results)
    report = build_report(
        ...
        runtime={
            ...
            "epochs": args.epochs,
            "folds": args.folds,
            "teacher_inner_folds": args.teacher_inner_folds,
            "seed": 0,
