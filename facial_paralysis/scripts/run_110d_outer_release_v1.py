#!/usr/bin/env python3
"""Execute the pinned one-shot protected PalsyNet 110D evaluation."""
...
def main() -> None:
    ...
    development_report, development_report_sha = _read_json(
    ...
    state = validate_outer_authorization(
        ...
        development_report_sha256=development_report_sha,
        audit=audit,
    )
    load_authorized_cache_records(
        args.palsynet_cache_root,
    ...
    result = run_protected_outer(
        dataset, gate, views, state=state, audit=audit
    )
    output = PROJECT_ROOT / DEFAULT_OUTER_REPORT_RELATIVE
    write_private_no_overwrite_json(output, result.report)
    report_sha = hashlib.sha256(output.read_bytes()).hexdigest()
    print(json.dumps({
        "schema_version": "110d_outer_release_receipt_v1",
