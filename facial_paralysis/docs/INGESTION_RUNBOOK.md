...
## A. HB labels arrive (≥40–60 takes) → supervised, validated severity
The unlock for a real clinical accuracy number.
...
3. Fine-tune from the champion warm-start with **leave-one-patient-out CV** (n is small):
   train the small heads on k−1 patients, test the held-out; report QWK + bootstrap CI
   (reuse `scripts/mayo_transfer_robust.py`'s bootstrap; `power_analysis.py` says n≈40–60
   gives ±0.10–0.14). **Use geometry-only (`deploy_config.json`) for the eye head** (MARLIN
   doesn't transfer); mouth can keep MARLIN or not (feature-importance: mouth ignores MARLIN).
4. Re-run `scripts/mayo_scorecard.py` — the `learned_severity` column becomes a *validated*
   grade instead of exploratory.
...
3. Re-run `scripts/mayo_binary.py` — now it reports real specificity (not just the saturated
   recall), so you can calibrate a threshold and trust the detector. Score both the champion
   (MARLIN) and geometry-only; pick whichever discriminates healthy-vs-palsy on the held-out
   in-domain split.
...
- `python3 autoresearch_fp/runner.py autoresearch_fp/best_config.json` (web champion still ~0.66)
