...
```bash
python3 facial_paralysis/tests/test_dynamic_window_data.py
python3 facial_paralysis/tests/test_nested_group_splits.py
```
...
/Users/researcher/.cache/facial-paralysis/mediapipe-py310/bin/python \
  ...
  --data-root /workspace/Mayo-Clinic/facial_paralysis/data/external/palsynet/data \
  --model-path /workspace/Mayo-Clinic/facial_paralysis/data/mediapipe_out/_models/face_landmarker.task \
  --identity-manifest /Users/researcher/.config/superpowers/worktrees/Mayo-Clinic/landmark-fusion/facial_paralysis/outputs/palsynet_identity_audit/identity_manifest.json \
  --output-root /workspace/Mayo-Clinic/facial_paralysis/data/external/palsynet/derived/clinical23_v2_windows
```
...
- [ ] The registry commit must be the direct child of `outer_producer_commit`, and its only changed path must be `facial_paralysis/configs/dynamic_landmark_experiment_registry.json`. Commit only that file and require a clean worktree. At one-shot preflight, require clean `HEAD` to be exactly this direct-child registry freeze commit; verify `HEAD^` equals the producer commit recorded in the registry and `git diff --name-only HEAD^ HEAD` contains exactly the registry path. Any intervening or later commit, producer-byte change, or registry change invalidates authorization and requires a new pre-prediction review/registry freeze; no outer prediction may be reused.
  ...
  ```bash
  OUTER_PRODUCER_COMMIT="$(python3 -c \
...
python3 \
  ...
  --cache-root /workspace/Mayo-Clinic/facial_paralysis/data/external/palsynet/derived/clinical23_v2_windows \
  ...
  --pretraining-run-root /Users/researcher/.config/superpowers/worktrees/Mayo-Clinic/landmark-fusion/facial_paralysis/outputs/dynamic_landmark/pretraining/formal \
...
python3 facial_paralysis/tests/test_dynamic_window_data.py
python3 facial_paralysis/tests/test_nested_group_splits.py
python3 facial_paralysis/tests/test_trajectory_features.py
python3 facial_paralysis/tests/test_dynamic_landmark_model.py
python3 facial_paralysis/tests/test_dynamic_landmark_benchmark.py
