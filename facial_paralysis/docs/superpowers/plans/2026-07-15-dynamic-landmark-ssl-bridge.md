...
- Producer provenance is `dynamic-landmark-bridge-producer-v2`: it binds normalized marshalled live executable semantics from the already-loaded producer modules together with owner-owned, single-link, non-group/world-writable, no-follow source-file snapshots. Each source is bounded to `4 MiB`, the set to `32 MiB`, every file descriptor and lexical path is rechecked after the full set is read, and an imported-code/on-disk mismatch therefore changes the digest rather than silently blessing either view.
...
- [ ] **Step 7: Implement the CLI with canonical, fail-closed paths.**
  ...
  It prints aggregate deidentified counts only. Every persisted producer digest uses the frozen v2 combination of loaded live-code semantics and the complete bounded/rechecked held source-file closure; neither only the imported code nor only later disk bytes can authorize a generation.
...
- [ ] Only after spec approval, obtain an independent code-quality/security review covering forged bundles/receipts, key/path/FD races, exact tree/manifest/ZIP resource bounds, producer live-code/file closure, close-all error handling, retained recovery state, mode confusion, source/group joins, scaler leakage, time/gap semantics, transfer allowlists, and output transactions.
