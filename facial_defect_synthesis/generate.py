...
Modes:
    ...
    python generate.py batch --per-category 20
        N diverse images for every category (large-scale dataset build).
...
def _run_one(spec: prompts.PromptSpec, fname: str, args) -> dict:
    ...
    if res.ok:
        ...
        record["file"] = fname
        print(f"  [ok]   {fname}  (retries={res.moderation_attempts}, "
...
def main() -> int:
    ...
    parser.add_argument("mode", choices=["catalog", "category", "batch"])
    parser.add_argument("--category", choices=prompts.CATEGORY_KEYS)
    parser.add_argument("--n", type=int, default=5, help="images for 'category' mode")
